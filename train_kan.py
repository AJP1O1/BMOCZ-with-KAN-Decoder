from BMOXZ_KAN import Encoder, Decoder, Channel
...
def train(
    ...
    """
    Trains an KAN-based neural network to detect the transmitted message. The transmitted 
    ...
    torch.save(model[0].state_dict(), PATH + f"\\bmoxz_kan_encoder.pth")
    torch.save(model[2].state_dict(), PATH + f"\\bmoxz_kan_decoder.pth")

    with open(PATH + f'\\model_kan_summary.txt', 'w') as file:
        file.write(
            f"""
            k = {k}
            hd = {hd}
...
def BLER(
    ...
    with open(PATH + f'\\bler_kan_summary.txt', 'w') as file:
...
def main():
    ...
    mode = 1 # 0 for training, 1 for BLER simulation, 2 for BER simulation
    ...
    if mode == 0:
        ...
        BER(
            ...
            hd = hidden,
            learnable_radii = learnable_radii,
            EbN0_range = [x for x in range(0, 13)],
            errors_needed = 1_500,
            fading_channel = False,
            disable_gpu = disable_gpu,
        )
